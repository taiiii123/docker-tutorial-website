...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. docker inspect で確認可能                               │
│     └─→ ホストにアクセスできる誰でも閲覧可能                 │
│                                                             │
│  2. docker history でビルド引数が見える                     │
│     └─→ ARGで渡した値はレイヤーに記録される                  │
│                                                             │
│  3. ログ出力                                                │
│     └─→ アプリケーションがログに環境変数を出力               │
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  [ ] Dockerfileにシークレットをハードコードしない             │
│  [ ] ARGでシークレットを渡さない                             │
│  [ ] .gitignore でシークレットファイルを除外                 │
│  [ ] .dockerignore でシークレットファイルを除外              │
│  [ ] docker history でシークレットが見えないことを確認        │
│  [ ] ログにシークレットが出力されないことを確認               │
│  [ ] シークレットのローテーション方針を定義                   │
│  [ ] アクセス制御と監査ログを設定                            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
