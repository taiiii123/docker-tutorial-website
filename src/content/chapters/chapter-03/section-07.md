...
```
┌─────────────────────────────────────┐
│     Layer 5: CMD ["npm", "start"]   │ (メタデータのみ)
├─────────────────────────────────────┤
│     Layer 4: COPY . /app            │ (ソースコード)
├─────────────────────────────────────┤
│     Layer 3: RUN npm install        │ (依存関係)
├─────────────────────────────────────┤
│     Layer 2: COPY package*.json ./  │ (package.json)
├─────────────────────────────────────┤
│     Layer 1: FROM node:20-alpine    │ (ベースイメージ)
└─────────────────────────────────────┘
```
...
キャッシュに同じ命令があるか?
  ...
  Yes → キャッシュを使用（高速）
    ↓
  No  → 新しいレイヤーを作成（時間がかかる）
         以降のすべてのレイヤーもキャッシュが無効化
```
