...
```
1. Docker CLI がコマンドを解析
        │
        ▼
2. REST API で Docker Daemon に送信
        │
        ▼
3. Docker Daemon がイメージの存在を確認
   └─▶ なければ Registry からプル
        │
        ▼
4. containerd にコンテナ作成を依頼
        │
        ▼
5. containerd が runc を呼び出し
        │
        ▼
6. runc がコンテナプロセスを作成
        │
...
### Mac の場合（Apple Silicon: M1/M2/M3/M4）

Apple Silicon Mac では、Apple の Virtualization.framework を使用してより効率的に動作します。
